...
void drawPoints()
{
    ...
    TIME = omp_get_wtime();
    //параллелим цикл. private(res) - делаем копию переменной res для каждого потока.
    //reduction(+:pointsInAreaCounter,generalPointsCounter) - накапливаем значения этих переменных
    //num_threads(THREAD_COUNT) - задаем количество потоков, выполняющих этот цикл
    #pragma omp parallel for private(res) reduction(+:pointsInAreaCounter,generalPointsCounter) num_threads(THREAD_COUNT)
...
{
    ofstream logs("tests_omp.txt", ios_base::app);
    double  xc,
            yc,
            col,
            res;
    ...
    for (int it = 0; it < iterationsCount; it++)
    {
        pointsInAreaCounter = 0;
        generalPointsCounter = 0;
        #pragma omp parallel for private(res,xc,yc,col) reduction(+:pointsInAreaCounter,generalPointsCounter) num_threads(threadNum)
...
void Display()
{
    ...
    cout << "Количество потоков: " << THREAD_COUNT << ". Время: " << TIME << endl;
...
int main(int argc, char **argv)
{
    ...
    while (!start)
    {
        ...
        switch (menuItem)
        {
            ...
            case 4:
                {
                    cout << "Введите количество потоков (1-" << omp_get_max_threads() << "): \n";
                    cin >> THREAD_COUNT;
